...
function installGlobalHook(window) {
  ...
  const hook = ({
    ...
    inject: function(renderer) { // 保存应用方法到hook_renderers
      var id = Math.random().toString(16).slice(2);
      hook._renderers[id] = renderer;
      var reactBuildType = hasDetectedBadDCE ?
        'deadcode' :
        detectReactBuildType(renderer);
      hook.emit('renderer', {id, renderer, reactBuildType}); // 触发hook的renderer事件
      return id;
    },
    _listeners: {},
    sub: function(evt, fn) {
    ...
    onCommitFiberRoot: function(rendererID, root) { // react commit phase will invoke the function
      const mountedRoots = hook.getFiberRoots(rendererID);
      const current = root.current;
      const isKnownRoot = mountedRoots.has(root);
      const isUnmounting = current.memoizedState == null || current.memoizedState.element == null;
      // Keep track of mounted roots so we can hydrate when DevTools connect.
      if (!isKnownRoot && !isUnmounting) { // 初次挂载
        mountedRoots.add(root);
      } else if (isKnownRoot && isUnmounting) { // 卸载
